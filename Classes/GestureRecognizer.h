...
struct Vec2d {
	...
	void operator =(const Vec2d& rOther) {
		x = rOther.x;
		y = rOther.y;
		return;
	}
...
public:
	...
	void load(string sFile) {
		...
		if (!in_file) {
			log("Error while loading gesture file: '%s'", sFile.c_str());
			return;
		}
		// TODO: reset first.
		stringstream ss;
